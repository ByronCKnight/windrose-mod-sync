...
-- [PATCH: windrose-mod-sync] Authority guard. See docs/campdepositreloaded-patch.md
--
-- Upstream says "install wherever the game's authoritative server logic actually
-- runs - not on clients". We ship this mod to the client install anyway, because
-- that same install IS the authority in singleplayer. So it has to decide at
-- runtime instead of at install time.
--
-- Without this, a client connected to a dedicated server would run the multipass
-- and briefly rewrite the interact component's owner pointer on client-side
-- objects - memory writes with no authoritative effect, and a window where the
-- game could read a pointer we lied about.
--
--   dedicated server .... authority -> runs
--   singleplayer ........ authority -> runs
--   Host Game server .... authority -> runs
--   multiplayer client .. no authority -> no-op
local authorityProbeLogged = false
local function hasAuthority(actor)
    local method, result

    local ok, v = pcall(function() return actor:HasAuthority() end)
    if ok and type(v) == "boolean" then
        method, result = "HasAuthority", v
    else
        -- Fallback: NM_Client == 3. Anything else (standalone, listen/dedicated
        -- server) is authoritative for this world.
        local ok2, nm = pcall(function() return actor:GetWorld():GetNetMode() end)
        if ok2 and tonumber(nm) then
            method, result = "GetNetMode=" .. tostring(nm), tonumber(nm) ~= 3
        else
            -- Neither probe worked. Refuse rather than poke memory on a guess.
            method, result = "none (failing closed)", false
        end
    end

    -- Log the verdict once so a silently-disabled mod is diagnosable from the log
    -- rather than looking like the mod is simply broken.
    if not authorityProbeLogged then
        authorityProbeLogged = true
        log("authority probe: %s -> %s", method, tostring(result))
    end
    return result
end
-- [/PATCH]
...
local function runMultipass(asc, pAbilityHandle, pOrigKey, pTargetData, pAppTag, pCurKey)
    ...
    -- [PATCH: windrose-mod-sync] see hasAuthority above
